...
class _CreatorProfileScreenState extends State<CreatorProfileScreen>
  ...
  Future<void> _loadFullProfile() async {
    ...
    // Also load reviews for this creator
    _loadReviews();
  }
  ...
  Future<void> _loadReviews() async {
    final user = widget.user ?? context.read<AuthService>().currentUser;
    if (user == null) return;
    final result = await context.read<AuthService>().fetchReviews(user.id);
    if (result != null && mounted) {
      setState(() {
        _avgRating = (result['avg_rating'] as num?)?.toDouble() ?? 0.0;
        _totalReviews = (result['total_reviews'] as num?)?.toInt() ?? 0;
  ...
  void _showReviewDialog(String creatorId) {
            ...
            ElevatedButton(
              onPressed: submitting
                  ...
                  : () async {
                      ...
                      final result =
                          await context.read<AuthService>().submitReview(
                                creatorId,
                                selectedRating,
                      ...
                      if (result != null) {
                        setState(() {
                          _avgRating = (result['avg_rating'] as num).toDouble();
                          _totalReviews =
                              (result['total_reviews'] as num).toInt();
                        });
                        _loadReviews();
  ...
  void _showReviewsBottomSheet(
      String creatorId, bool isOwner, bool isLoggedIn) {
              ...
              child: _reviews.isEmpty
                  ...
                  : ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final name = review['reviewer_name'] ?? 'مجهول';
                        final rating = (review['rating'] as num?)?.toInt() ?? 0;
                        final comment = review['comment'] as String?;
                        final avatarUrl = review['reviewer_avatar'] as String?;
            ...
            if (!isOwner && isLoggedIn)
              Padding(
                padding: EdgeInsets.fromLTRB(
                    16, 8, 16, MediaQuery.of(ctx).padding.bottom + 24),
                child: SizedBox(
                  width: double.infinity,
                  child: ElevatedButton.icon(
                    onPressed: () {
                      Navigator.pop(ctx);
                      _showReviewDialog(creatorId);
                    },
                      ...
                      if (_isLoadingProfile)
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            GestureDetector(
                              onTap: () {
                                // Everyone: open bottom sheet with reviews list
                                _showReviewsBottomSheet(
                                    displayUser.id, isOwner, isLoggedIn);
                              },
...
class ReviewsTab extends StatefulWidget {
  ...
  final void Function(double avgRating, int total) onReviewSubmitted;
          ...
          child: widget.reviews.isEmpty
              ...
              : ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final name = review['reviewer_name'] ?? 'Anonymous';
                    final rating = (review['rating'] as num?)?.toInt() ?? 0;
                    final comment = review['comment'] as String?;
                    final avatarUrl = review['reviewer_avatar'] as String?;
                    final date = review['created_at'] != null
